...
function mk_xrp_set_settings() {
    ...
    register_setting( 'writing', 'mk_xrp_enable_xmlrpc', 'intval' );
 	...
 	add_settings_section('mk_xrp',
		__('XML-RPC 远程发布协议','mk_xrp_filter'),
		'mk_xrp_section_callback_function',
		'writing');

 	add_settings_field('status',
		'<label for="mk_xrp_enable_xmlrpc">'.__('开启','mk_xrp_textdomain').'</lable>',
		'mk_xrp_show',
		'writing',
		'mk_xrp');
...
function mk_xrp_section_callback_function() {
  echo '<p>'.__('启用 WordPress、Movable Type、MetaWeblog 和 Blogger 的 XML-RPC 发布协议，用于桌面客户端软件，远程网站发布文章。','mk_xrp_textdomain').'</p>';
}  
