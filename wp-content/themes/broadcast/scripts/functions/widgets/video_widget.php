...
class lt_video_widget extends WP_Widget {
	...
	function lt_video_widget() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'lt_video_widget', 'description' => __('嵌入视频到侧边栏.', 'skyali') );

		/* Widget control settings. */
		//$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'lt_300x250_widget' );

		/* Create the widget. */
		$this->WP_Widget( 'lt_video_widget', __('LT - 嵌入视频', 'skyali'), $widget_ops);
	}
	...
	function widget( $args, $instance ) {
       ...
       ?>
       <?php printf( __('%1$s', 'skyali'), $video_embed ); ?>
	   <?php
	...
	function form( $instance ) {
        ...
        <p>
          <label for="<?php echo $this->get_field_id( 'video_embed' ); ?>"><?php _e('Video Embed Code:', 'skyali'); ?></label>
