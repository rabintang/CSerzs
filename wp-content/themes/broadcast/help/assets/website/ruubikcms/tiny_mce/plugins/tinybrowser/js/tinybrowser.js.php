...
elseif($mainpage && $_GET['feid'] != '')
	{?>
	function selectURL(url, filesize) { // FILELINK: Added passing filesize
	opener.document.getElementById("<?php echo $_GET['feid']; ?>").value = url;
	if (opener.document.getElementById("filesizeid")) {opener.document.getElementById("filesizeid").value = filesize;} // FILELINK: Added line for passing filesize
	// Set img source of element id, if img id exists (format is elementid + "img")
	if(opener.document.getElementById("<?php echo $_GET['feid']; ?>img") != null && typeof(opener.document.getElementById("<?php echo $_GET['feid']; ?>img")) != "undefined" && opener.document.getElementById("<?php echo $_GET['feid']; ?>img").src.length != 0)
	   {
