...
{
	global $dbh;
	if ($onlypublished == TRUE) $statusquery = ' AND STATUS = 1';
	else $statusquery = '';
...
{
	return query_single("SELECT ".$column." FROM ".$table." WHERE levelnum = 1 AND status = 1 ORDER BY ordernum LIMIT 1");
}
...
function clean_url($pageurl, $table='page') 
{
	...
	if ($url_suffix != "") $url_suffix = '.'.$url_suffix; 
	$urlbase = '/'.($siteroot != "" ? $siteroot.'/' : '').($table == 'extrapage' ? 'extra/' : '').'index.php/';
	if ($level > 1) $mother = query_prep("SELECT mother FROM ".$table." WHERE pageurl = ?", array($pageurl));
	if ($level <= 1) {
		$urlrest = $pageurl.$url_suffix;
	} elseif ($level == 2) {
		$urlrest =  $mother.'/'.$pageurl.$url_suffix;
	} elseif ($level == 3) {
		$urlrest = query_prep("SELECT mother FROM ".$table." WHERE pageurl = ?", array($mother)).'/'.$mother.'/'.$pageurl.$url_suffix;;
	} else {
		return FALSE;
	}
...
function snippetstr($text,$length,$tail="...") {
	if ($length < 1 OR $length > 10000 OR !is_numeric($length)) $length = 110; // default invalid to 110 chars
    $text = trim($text);
    $txtl = strlen($text);
    if($txtl > $length) {
