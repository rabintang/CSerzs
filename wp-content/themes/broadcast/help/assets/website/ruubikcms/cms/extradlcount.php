...
if (query_single("SELECT COUNT(*) FROM extra_dl_count") != 0) {
	if (isset($_GET['p'])) {
		if (isset($_GET['d'])) {
			// some CSRF protection
			if (!valid_csrf_token($_GET['token'])) die(NOTALLOWED);
			// delete selected count
			$stmt = $dbh->prepare("DELETE FROM extra_dl_count WHERE filename = ?");
                ...
                <div id="rightDiv">
					...
					<div id="rightContent">					
						...
						<div id="newsAdmin">
							...
							<h2><?php echo EXTRADOWNLOADSTATS;?></h2>
								...
								$total = query_single("SELECT COUNT(*) FROM extra_dl_count");
								...
								$sql = "SELECT * FROM extra_dl_count ORDER BY ".$order.$desc." LIMIT ".$start.", ".$rowsperpage;
								foreach ($dbh->query($sql) as $row) {
									echo '<tr><td>'.$row['filename'].'</td><td>'.$row['downloads'].'</td><td>'.$row['count_started'].'</td><td>'.$row['last_dl'].'</td><td><a href="extradlcount.php?p='.$row['filename'].'&amp;d=1&amp;token='.$token.'">'.RESET.'</a></td></tr>';
								}

								echo '</table>';
								// print the navigation links for pagination
								if ($lastpage > 1) echo '<p>'.$prev.$nav.$next.'</p>';
								echo '<p><a href="extradllog.php">'.EXTRADOWNLOADLOG.'</a></p>';
								?>
