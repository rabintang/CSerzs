...
if (isset($_POST['save'])) {
	...
	if ($_POST['mother'] == '---notinmenu---') {
		...
		$levelnum = query_prep("SELECT levelnum + 1 FROM page WHERE pageurl = ?", array($_POST['mother']));
	} else {
		$levelnum = 1;
	}
	...
	if (!isset($_GET['p']) AND isset($_GET['n'])) {
		//echo $_POST['pageurl'];
		if (!empty($_POST['pageurl'])) $newpageurl = get_unique_url($_POST['pageurl'], 0); // make pageurl from URL if given
		else $newpageurl = get_unique_url($_POST['name'], 0); // otherwise make pageurl from name (function returns "no-name" if not $_POST['name'])
		
		$_POST['ordernum'] = get_next_ordernum($_POST['mother']);
	} 
	
	// page already exists
	if (isset($_GET['p'])) {
	
		$oldlevel = query_prep("SELECT levelnum FROM page WHERE pageurl = ?", array($_GET['p']));
		$oldmother = query_prep("SELECT mother FROM page WHERE pageurl = ?", array($_GET['p']));
		...
		if ($_POST['mother'] != $oldmother) {
			...
			/*if (has_children($_GET['p']) AND $levelnum != $oldlevel) {
		...
		if ($_POST['pageurl'] != $_GET['p']) {
			if (!isset($error)) {
				...
				$stmt->execute();
				if ($_POST['pageurl']) $newpageurl = get_unique_url($_POST['pageurl'], 0); // make pageurl from URL if given
				else $newpageurl = get_unique_url($_POST['name'], 0); // otherwise make pageurl from name (function returns "no-name" if not $_POST['name'])
				$pageurlchanged = TRUE;
				// also change mother for child pages
				$stmt = $dbh->prepare("UPDATE page SET mother = '$newpageurl' WHERE mother = ?");
				$stmt->bindParam(1, $_GET['p']);
				$stmt->execute();
			}
		} else {
	...
	if (!isset($error)) {
		...
		$stmt = $dbh->prepare("INSERT OR REPLACE INTO page (pageurl, name, title, header1, content, levelnum, ordernum, description, keywords, mother, image1, image2, pagetype, extracode, status, updater, updated, creator) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		...
		$stmt->bindParam(17, date("Y-m-d H:i:s"));
		$stmt->bindParam(18, $_POST['creator']);
		$stmt->execute();
				
		if (isset($motherchanged)) refresh_pageorder($oldmother);
...
$pagelist = pages_for_select(2);

if (isset($_GET['p'])) {
	// get pagedata from database
	$page = get_page_data($_GET['p']);
	if ($site['clean_url'] >= 1) {
	...
	if (has_children($_GET['p'])) $children = TRUE;
	else $children = FALSE;
	...
	if (isset($_GET['d'])) {
		...
		if ($children) {
			...
			if ($_SESSION['level'] >= 4 OR $page['creator'] == $_SESSION['uid']) {
				...
				else $redirect = '?p='.query_single("SELECT pageurl FROM page WHERE levelnum = ".$page['levelnum']." AND mother = '".$page['mother']."' ORDER BY levelnum LIMIT 1");
		...
		if ($_SESSION['level'] >= 4) {
			if ($page['ordernum'] == 1) {
				...
				$stmt = $dbh->prepare("UPDATE page SET ordernum = ? WHERE ordernum = ? - 1 AND mother = ?");
				...
				$stmt = $dbh->prepare("UPDATE page SET ordernum = ? - 1 WHERE pageurl = ?");
				$stmt->bindParam(1, $page['ordernum']);
                ...
                <div id="rightDiv">
					...
					<div id="rightContent">
						<div id="pageAdmin">
							...
							<table cellspacing="0" cellpadding="0" border="0">
								...
								<tr>
									...
									<td class="tdPageAdminCenter"><input type="text" readonly="readonly" name="pagelink" value="<?php if (isset($pagelink)) echo ec($pagelink);?>" /></td>
								...
								<!--<tr>
									<td class="tdPageAdminLeft"><?php echo PAGETYPE;?></td>
									<td class="tdPageAdminCenter">
										<select name="pagetype">
										<?php foreach ($pagetype as $key => $value) {
											echo '<option value="'.$key.'"'.(isset($page['pagetype']) AND $key == $page['pagetype'] ? ' selected="selected"' : '').'>'.$value.'</option>';
										} ?>
										</select>
									</td>
									<td class="tdPageAdminRight"><a href="#" class="tooltip" title="<?php echo H_PAGETYPE;?>"><img src="images/help.gif" class="imgover" alt="" title="<?php echo H_PAGETYPE;?>" /></a></td>
								</tr>-->
								
							</table>
							
							<?php if ($children) echo '<input name="mother_hidden" type="hidden" value="'.$page['mother'].'" />'; // disabled select does not post value?>
							<input name="creator" type="hidden" value="<?php if(isset($page['creator'])) echo ec($page['creator']);?>" />
						...
						<div id="xtrArea">
							...
							<textarea name="extracode" cols="2" rows="2"><?php if (isset($page['extracode'])) echo stripslashes_gpc(htmlentities($page['extracode']));?></textarea></div>
						</div>
					...
					<div id="contentManagement">
						<div id="tinyMCE">
							<textarea cols="63" rows="20" name="tinyMCE" class="tinyMCE" id="tinyMCEarea"><?php if (isset($page['content'])) echo htmlentities($page['content']);?></textarea>
						</div>
