		...
		foreach(split("\n", $csvcontent) as $line) {
			...
			if ($lines > 0) {
				if (count($linearray) == 9) {
					...
					$stmt->bindParam(2, sha1($linearray[1]));
						...
						<div id="newsAdmin">
							...
							if (isset($_POST['search']) AND !empty($_POST['keyword'])) {
								...
								while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
									if ($counter == 0) {
										echo '<p>'.SEARCHRESULTSFOR.' "<strong>'.stripslashes(htmlentities($_POST['keyword'])).'</strong>"</p></p><p><a href="extrausers.php"><< '.EXTRAUSERS.'</a></p>';
								...
								else echo '<p>'.NOSEARCHRESULTS.' "<strong>'.stripslashes(htmlentities($_POST['keyword'])).'</strong>"</p><p><a href="extrausers.php"><< '.EXTRAUSERS.'</a></p>';
							...
							<input type="text" name="keyword" value="<?php if (isset($_POST['keyword'])) echo stripslashes(htmlentities($_POST['keyword']));?>" />
